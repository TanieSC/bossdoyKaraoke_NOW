   ...
   public bool MakeTransparent;
	...
	public System.Drawing.Image RGBImage
    {
        get
        {
            ...
            Stream stream = temp as Stream;
            Bitmap myBitmap = GraphicUtilDX.StreamToBitmap(ref stream, CDG_FULL_WIDTH, CDG_FULL_HEIGHT);
            if (MakeTransparent)
            {
	...
	private void reset()
	{
      Array.Clear(m_pixelColours, 0, (int)m_pixelColours.LongLength);
		Array.Clear(m_colourTable, 0, (int)m_colourTable.LongLength);
		...
		if ((m_pSurface.rgbData != null)) {
         Array.Clear(m_pSurface.rgbData, 0, (int)m_pSurface.rgbData.LongLength);
		}
...
public class ISurface
{
	...
	public int MapRGBColour(int red, int green, int blue)
	{
      ...
      int  grayScale = (int)((red * .3) + (green * .59) + (blue * .11));
