    ...
    public class Main_Form
    {
        ...
        public void Initialize(Control playerWindow)
        {
            ...
            /* if (Un4seen.Bass.Utils.Is64Bit)
             ...
             bool b = Bass.LoadMe(targetPath);
             bool fx = BassFx.LoadMe(targetPath);
             bool b1 =BassMix.LoadMe(targetPath);
             bool b2 = BassAsio.LoadMe(targetPath);*/
            InitTimer();
                ...
                if (!m_IsPlayingCdg && m_IsPlayingVideo)
                {
                    ...
                    if (m_vlc.MediaEnded)
                    {
                        ...
                        else
                        {
                            ...
                            m_vlc.LoadDefaultVideoBackGround();
                            m_timerVideo.Start();
        ...
        private void RMS(out double dbLevelL, out double dbLevelR)
        {
            ...
            if (Player.IsAsioInitialized)
            {
                float fpeakL = BassAsio.BASS_ASIO_ChannelGetLevel(false, BassAsioDevice.asioChannel);
                float fpeakR = BassAsio.BASS_ASIO_ChannelGetLevel(false, BassAsioDevice.asioChannel + 1);
        ...
        void Workers_Completed(object sender, RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                if (getSEARCHDIRorTEXTState == SearchAndLoad.SEARCH_DIRECTORY)
                {
                    ...
                    File.WriteAllLines(fPath + filename + ".bkN", filaPathArray);
                }
