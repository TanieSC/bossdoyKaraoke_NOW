...
namespace bossdoyKaraoke_NOW.Models
{
    ...
    {
        public static void QueueWorkItem<Tin, Tout>(Queue<QueueItem<Tin>> queue, Tin inputArgument, Func<DoWorkArgument<Tin>, Tout> doWork, Action<WorkerResult<Tout>> workerCompleted)
        {
            ...
            {
                workerCompleted?.Invoke(new WorkerResult<Tout>((Tout)args.Result, args.Error));
                queue.Dequeue();
                if (queue.Count > 0)
                {
